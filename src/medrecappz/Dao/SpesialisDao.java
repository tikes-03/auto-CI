...
import medrecappz.Interface.SpesialisInterface;
//import medrecappz.Interfaces.SpesialisInterface;
...
public class SpesialisDao implements SpesialisInterface {
    ...
    public void insertSpesialis(Spesialis s) throws SQLException {
        try {
            ...
            ps.setString(3, s.getTarifKonsul());
            if ((s.getIdSpesialis().equals("")) || (s.getNmSpesialis().equals(""))) {
                hasilInsert = "Data tidak boleh kosong!";
            } else {
                ps.executeUpdate();
                ps.close();
                hasilInsert = "ok";
    ...
    public void updateSpesialis(Spesialis s, String idSpesialis) throws SQLException {
        try {
            ...
            ps.setString(2, s.getTarifKonsul());
            ps.setString(3, idSpesialis);
            if ((s.getTarifKonsul().equals("")) || (s.getNmSpesialis().equals(""))) {
                hasilUpdate = "Data tidak boleh kosong!";
            } else {
                ps.executeUpdate();
                ps.close();
    ...
    public List getAllSpesialisById(String idSpesialis) throws SQLException {
        try {
            ...
            while (rs.next()) {
                ...
                s.setTarifKonsul(rs.getString("tarif_konsul"));
                list.add(s);
            }
            ps.close();
            rs.close();
